...
def _sync_inventory(hashes: list[str]) -> None:
    ...
    client.push_inventory(hashes)


def _private_download_options(job: dict, url: str) -> tuple[dict[str, str] | None, bool]:
    grant = job.get("downloadGrant")
    if not grant:
        return None, True
    header = str(job.get("downloadGrantHeader") or "").strip().lower()
    if header != "x-arcenciel-link-grant":
    ...
    target = urlparse(url)
    backend = urlparse(client.BASE_URL)
    if target.scheme != backend.scheme or target.hostname != backend.hostname or target.port != backend.port:
        raise RuntimeError("Private download URL does not match the configured ArcEnCiel host")
    if target.scheme != "https" and not client.DEV_MODE:
    ...
    return {"X-ArcEnCiel-Link-Grant": grant.strip()}, False
...
def _write_info_json(meta: dict, sha_local: str, preview_name: str | None, model_path: Path):
    info = {
        "schema": 1,
    ...
    sd_file.write_text(json.dumps(sd_meta, indent=2, ensure_ascii=False), encoding="utf-8")


def _write_html(meta: dict, preview_name: str | None, model_path: Path):
    title = escape(str(meta.get("modelTitle") or "ArcEnCiel Model"))
    html = dedent(f"""
    ...
    if meta.get("aboutThisVersion"):
        html += f"<h2>About this version</h2><p>{escape(str(meta['aboutThisVersion']))}</p>"
    ...
    sha256: {escape(str(meta.get("sha256") or ""))}</small></p></html>
    """
    (model_path.parent / (model_path.stem + ".arcenciel.html")).write_text(html, encoding="utf-8")
    ...
    while True:
        ...
        try:
            ...
            if url_raw and not url_raw.startswith(("http://", "https://")):
                ...
                root = client.BASE_URL.split("/api/")[0].rstrip("/")
            ...
            try:
                ...
                client.report_progress(job["id"], state="ERROR", message=str(exc))
                continue
            ...
            if not _enough_free_space(dst_path.parent):
                client.report_progress(job["id"], state="ERROR", message=f"Less than {MIN_FREE_MB} MB free")
                continue
            ...
            def _progress_cb(frac: float):
                ...
                client.report_progress(job["id"], progress=pct)
...
def generate_sidecars_for_existing():
    ...
    cache = _load_cache()
    model_files = {v["hash"]: Path(k) for k, v in cache.items() if Path(k).exists()}
    if not model_files:
        return
